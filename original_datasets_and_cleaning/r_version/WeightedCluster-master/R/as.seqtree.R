  ...
  as.seqtreeDTNBuildNode <- function(ind, vardis, depth, current) {
    ...
    for (p in current:ncluster) {
      ...
      if (length(SplitLabels) == 2) {
        ...
        # print(diss)
        lSCres <- TraMineRInternalWeightedInertiaDist(
          diss, as.integer(nrow(diss)),
          as.integer(FALSE), as.integer(ind[bestSpl$variable]), as.double(weights),
          as.integer(FALSE)
        )
        rSCres <- TraMineRInternalWeightedInertiaDist(
          diss, as.integer(nrow(diss)),
          as.integer(FALSE), as.integer(ind[!bestSpl$variable]), as.double(weights),
          as.integer(FALSE)
        )
        ...
        bestSpl$spl <- TraMineRInternalSplitInit(p - 1,
          index = 1:2,
